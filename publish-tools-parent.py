"""Create the /tools/ parent page (or update if exists), then republish the IPC page as a child."""
import base64, json, urllib.request

# 1. Create /tools/ parent landing page
parent_content = """<!-- wp:paragraph -->
<p>Free betting tools from PropsBot.AI. Convert odds to implied probability, calculate parlay payouts, find vig and hold, and more — every tool below is free, no signup required, and built for sharp prop bettors.</p>
<!-- /wp:paragraph -->
...
<!-- wp:list-item --><li><strong><a href="/tools/parlay-calculator/">Parlay Calculator</a></strong> — combine multiple bets, see total payout and implied probability</li><!-- /wp:list-item -->
</ul>
<!-- /wp:list -->
...
<!-- wp:paragraph -->
<p>Every tool on this page is designed for sports prop bettors who want to evaluate lines, build parlays, and identify edges. They're free, fast, mobile-friendly, and built with the same data PropsBot's AI uses to score props every day. When you're ready to see <strong>AI Confidence Scores</strong> on every player prop, head to the <a href="https://app.propsbot.ai/">PropsBot app</a>.</p>
<!-- /wp:paragraph -->
...
content_b64 = base64.b64encode(parent_content.encode('utf-8')).decode('ascii')
payload = json.dumps({
    'title': 'Free Betting Tools — Calculators for Prop Bettors | PropsBot.AI',
    'slug': 'tools',
    'content': content_b64,
}).encode('utf-8')
